...
const ModalFollowing: FC<IPropsModal> = ({ handleCloseModal, openModal }) => {
  const { followings } = useFollowStore((state) => state);

  const queryResults = useQueries({
    queries: followings.map((id) => ({
      queryKey: ['user', id],
      queryFn: () => getUser(id),
    })),
  });
  ...
  return (
    <ModalLayout handleCloseModal={handleCloseModal} openModal={openModal}>
      <div className={`animation transition-all ${openModal ? 'scale-100' : 'scale-105'}`}>
        <div className='max-w-[360px] sm:min-w-[400px] max-h-[400px] rounded-2xl overflow-y-scroll scrollbar-hide relative bg-white'>
...
export default memo(ModalFollowing);
